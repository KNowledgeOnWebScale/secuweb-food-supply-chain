...
its canonicalized representation. The blockchain responds with metadata about the transaction 
...
includes the URI of the VC (`VC_URI`).
Using `VC_URI`, Actor B can lookup the necessary information for the verification of the VC.

The verification process involves four critical steps:

1. Retrieval:
      - Actor B looks up the transaction ID to obtain the necessary information:
            i) URI to the VC on Actor A's Solid Pod;
            ii) the hash of the canonized VC that was computed by Actor A
      - Actor B fetches the VC from Actor A’s Solid Pod using the VC URI.
2. Blockchain Query: Actor B looks up `VC_URI` on the BC. The lookup result consists of, a.o., the hash of the corresponding anchoring transaction from the blockchain, the hash of the canonized VC.
3. Local Hash Computation: Actor B canonicalizes the fetched VC and locally computes the hash value.
4. Hash Comparison: Actor B compares the locally computed hash with the one recorded on the blockchain.
...
Through this decentralized verification process, the system achieves a trustless
